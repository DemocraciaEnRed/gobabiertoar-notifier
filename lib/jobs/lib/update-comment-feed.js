var db = require('../../db')
var log = require('debug')('democracyos:notifier:update-comment-feed')
var ObjectId = require('mongojs').ObjectId

var jobName = 'update-comment-feed'
    ...
    db.topics.findOne({_id: ObjectId(data.id)}, function (err, topic) {
      ...
      db.feeds.findOne({ forum: ObjectId(topic.forum) }, function (err, feed) {
        ...
        db.feeds.save(feed, function (err, feed) {
          ...
          log('Saved comment feed for published topic %s', feed.topic)
          done()
        })
      })
    })
